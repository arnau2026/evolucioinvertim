...
h1 {font-size:2rem!important; color:#132238; letter-spacing:-.03em}
.live {display:inline-block;padding:6px 11px;border-radius:999px;color:#087f5b;background:#e9fbf4;border:1px solid #b7efd8;font-size:.82rem;font-weight:700}
...
[data-testid="stMetricValue"] {color:#172033;font-weight:750}
div[role="radiogroup"] {gap:.35rem}
div[role="radiogroup"] label {background:#f1f5f9;border:1px solid #e2e8f0;border-radius:10px;padding:.35rem .8rem;min-width:55px;justify-content:center}
div[role="radiogroup"] label:has(input:checked) {background:#143d59!important;border-color:#143d59!important;color:#fff!important}
div[role="radiogroup"] label:has(input:checked) p {color:#fff!important;font-weight:750!important}
div[role="radiogroup"] label:has(input:checked) [data-testid="stMarkdownContainer"] {color:#fff!important}
div[role="radiogroup"] label:has(input:checked) div[role="radiogroup"] {color:#fff!important}
...
try:
    ...
    fig.update_layout(height=555, margin=dict(l=20, r=20, t=60, b=20), title=dict(text=f"Rentabilidad acumulada {period} · Última sesión: {curve.index[-1]:%d/%m/%Y}", x=.01), paper_bgcolor="#fff", plot_bgcolor="#f8fafc", font=dict(color="#334155"), xaxis=dict(showgrid=False, linecolor="#dbe3eb"), yaxis=dict(title="Rentabilidad acumulada", ticksuffix="%", gridcolor="#e5ebf1"), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1), hovermode="x unified")
